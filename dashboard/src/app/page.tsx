...
export default function DashboardPage() {
  ...
  const { fetchData, getCached } = useCache<QueueStats[]>("queue_stats", () => api.getQueueStats(), 30000);
  ...
  }, [fetchData, getCached, stats.length]);
  ...
  const StatBox = ({ label, value, icon: Icon, accent }: { label: string; value: number; icon: any; accent: string }) => (
  ...
  return (
    <div className="space-y-6 font-mono">
      <div className="flex items-center justify-between">
        ...
        <Button onClick={fetchStats} variant="outline" size="sm" className="border-[#333] text-[#666] hover:text-[#00ff00] hover:border-[#00ff00] bg-transparent">
          REFRESH
        </Button>
      </div>
